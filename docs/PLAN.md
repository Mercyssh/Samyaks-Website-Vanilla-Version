...
- 🔄 **P4 Three.js scenes** — ✅ Landing (`js/three/renderer.js` shared stage + `js/three/landing-scene.js`: loads `landing.glb`, framed camera, offscreen-pause; **pinned** scroll scrub — hand recedes+fades, stack flips 180° (cover backface-culled), left/right fan out; icon planes hover-lit + clickable; no pointer parallax; DOM `.landing__center` hidden). Debug GUI at `?debug` (lil-gui + OrbitControls: camera pos/target/fov, progress scrub, tuning, log-camera). · ⬜ Abilities (click-to-flip) · ⬜ Journey (camera travel). Choreography in `docs/scene-specs.md`.
